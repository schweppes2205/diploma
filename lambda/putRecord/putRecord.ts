...
class QueryParamHelper {
    constructor(public resource: string, public record: string) { }
}
...
export const handler = async (event: any) => {
    ...
    const restPutParams: QueryParamHelper = event.queryStringParameters;
    ...
    if (restPutParams.resource && restPutParams.record) {
        try {
            let ddbQueryParams: AWS.DynamoDB.PutItemInput = {
                ...
                Item: JSON.parse(restPutParams.record),
            };
